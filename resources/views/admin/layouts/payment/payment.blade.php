...
@include('admin.layouts.sidebars.transaction')
...
<div class="container">
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            @if(!empty($trans) && $trans->count())
                @foreach($trans as $key => $value)
                    <tr>
                        <td>{{ $value->id }}</td>
                        <td>{{ $value->pay_price }}</td>
                        <td>{{ $value->pay_user_id }}</td>
                        <td>{{ $value->name }}</td>

                        <td>{{ $value->pay_transaction_id }}</td>
                        <td>
                            @if ($value->pay_paysta_status_id ==1)
                            Pending
